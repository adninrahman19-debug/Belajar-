...
import { TrackingData } from '../services/realtime';
...
const LiveMap: React.FC<LiveMapProps> = ({ trackingData }) => {
  ...
  useEffect(() => {
    ...
    if (!mapRef.current) {
      ...
      // Using CartoDB Dark Matter tiles which look very "hacker-ish"
  ...
  useEffect(() => {
    ...
    if (trackingData && mapRef.current && L) {
      ...
      if (!markerRef.current) {
        // Custom Marker HUD Style
        const customIcon = L.divIcon({
          className: 'custom-div-icon',
          html: `
            <div class="relative">
              <div class="absolute -inset-8 border border-cyan-500/20 rounded-full animate-ping"></div>
              <div class="absolute -inset-4 border border-cyan-500/40 rounded-full animate-pulse"></div>
